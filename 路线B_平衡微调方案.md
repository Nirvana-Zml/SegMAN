...
## 12. balanced-v2 重训（bowl 导向，2026-05-23）

**目的**：在保持 balanced 路线 **mIoU / window** 优势的同时，用算法改动抬高 **bowl**（针对 §8.6：background/cup 混淆）。**不修改** `segman_b_trans10k_lass.py` 与 fix5k 权重目录；交付仍用 **fix5k**。

### 12.1 相对 balanced v1 的改动

| 项 | balanced v1 (10k) | **balanced-v2** |
|----|-------------------|-------------------|
| LASS stages | [1, 2] | **[0, 1, 2]**（浅层弱 lr） |
| ltab / rsm init | 0.05 / 0.3 | **0.06 / 0.35** |
| Dice | 0.4 | **0.15** |
| cup / bowl CE | 1.05 / 1.0 | **1.0 / 1.10** |
| 新损失 | — | **BowlAntiCupLoss** 0.25（GT=bowl 压低 P(cup)） |
| boundary | 0.15 | **0.18** |
| max_iters | 10000 | **8000** |
| work-dir | `balanced10k` | **`balanced_v2`** |

**代码**：`segmentation/mmseg/models/losses/bowl_anti_cup_loss.py`  
**配置**：`local_configs/segman_trans/segman_b_trans10k_lass_balanced_v2.py`  
**fix5k 备份**：`local_configs/segman_trans/recipes/segman_b_trans10k_lass_fix5k.py`；v1 归档 `archive/segman_b_trans10k_lass_balanced_v1.py`

### 12.2 训练命令（Docker，`segmentation`）

**A. 从基线 80k 训（主实验）**

```bash
mkdir -p outputs/trans10k_lass_mmscope_balanced_v2

nohup bash scripts/train_route_b_balanced_v2.sh \
  > outputs/trans10k_lass_mmscope_balanced_v2/train.log 2>&1 &
```

**B. 从 fix5k 再训 8k（两阶段对照，可选）**

```bash
nohup bash scripts/train_route_b_balanced_v2.sh two_phase \
  > outputs/trans10k_lass_mmscope_balanced_v2/train_two_phase.log 2>&1 &
```

**评测 sweep（2k/4k/6k/8k）**

```bash
bash scripts/test_balanced_v2_sweep.sh
```

### 12.3 验收（相对 balanced10k / 基线）

| 指标 | 目标 |
|------|------|
| mIoU | ≥ **80.71%**（理想 ≥ **81.0%**） |
| bowl IoU | ≥ **78.91%**（理想 ≥ **79.5%**） |
| window IoU | 不明显低于 balanced10k（**≥ 80%** 参考） |

### 12.4 实测：iter_2000（val 1000，2026-05-23）

**权重**：`outputs/trans10k_lass_mmscope_balanced_v2/iter_2000.pth`  
**配置**：`segman_b_trans10k_lass_balanced_v2.py`

#### 12.4.1 总体指标（与基线 / fix5k / balanced10k）

| 指标 | 基线 | fix5k | balanced10k | **v2 @ 2k** | v2−基线 | v2−fix5k | v2−balanced10k |
|------|------|-------|-------------|-------------|---------|----------|----------------|
| **mIoU** | 80.71 | 80.84 | 81.76 | **81.58** | **+0.87** | **+0.74** | −0.18 |
| aAcc | 96.07 | 95.92 | 96.15 | **96.05** | −0.02 | +0.13 | −0.10 |
| mAcc | 88.14 | 87.45 | 87.71 | **88.18** | +0.04 | +0.73 | +0.47 |

**相对 fix5k（正式交付权重）**：v2@2k **mIoU +0.74**、**mAcc +0.73**；aAcc 略高 +0.13。

#### 12.4.2 各类 IoU（%）— 与 fix5k 全量对比

| 类别 | 基线 | fix5k | balanced10k | **v2 @ 2k** | v2−fix5k | v2−基线 | 相对 fix5k |
|------|------|-------|-------------|-------------|----------|---------|------------|
| background | 96.71 | 96.37 | 96.45 | 96.44 | +0.07 | −0.27 | ≈ |
| box | 71.47 | 71.44 | 71.86 | **72.71** | **+1.27** | +1.24 | ↑ |
| bottle | 87.77 | 86.51 | 88.20 | 86.46 | −0.05 | −1.31 | ≈ |
| **window** | 66.62 | 76.27 | 82.91 | **82.76** | **+6.49** | **+16.14** | **↑** |
| eyeglass | 92.85 | 90.81 | 92.01 | 89.58 | −1.23 | −3.27 | ↓ |
| freezer | 73.90 | 73.46 | 73.48 | **73.97** | +0.51 | +0.07 | ≈ |
| jar_kettle | 84.04 | 82.06 | 83.89 | **82.62** | +0.56 | −1.42 | ≈ |
| door | 75.04 | 72.73 | 74.40 | **72.97** | +0.24 | −2.07 | ≈ |
| cup | 90.91 | 90.19 | 90.96 | **90.97** | **+0.78** | +0.06 | ↑ |
| wall | 82.72 | 82.77 | 83.95 | **83.57** | +0.80 | +0.85 | ↑ |
| **bowl** | 78.91 | 80.07 | 74.31 | **80.32** | **+0.25** | **+1.41** | **↑** |
| shelf | 67.61 | 67.44 | 68.73 | 66.59 | −0.85 | −1.02 | ↓ 略 |
| **mIoU** | 80.71 | 80.84 | 81.76 | **81.58** | **+0.74** | **+0.87** | **↑** |

**IoU 相对 fix5k（\|Δ\|>0.2%）**：↑ **5**（window、box、cup、wall、bowl）；↓ **2**（eyeglass、shelf）；其余 ≈。

**相对 balanced10k**：bowl **+6.01**、window **−0.15**（基本持平）；mIoU **−0.18**。

#### 12.4.3 各类 Acc（%）— 与 fix5k 对比

| 类别 | 基线 | fix5k | **v2 @ 2k** | v2−fix5k |
|------|------|-------|-------------|----------|
| background | 98.22 | 98.33 | 98.03 | −0.30 |
| box | 80.57 | 79.44 | **82.12** | **+2.68** |
| bottle | 89.93 | 87.97 | 87.91 | −0.06 |
| window | 90.77 | 86.71 | 86.41 | −0.30 |
| eyeglass | 95.92 | 96.04 | 97.52 | +1.48 |
| freezer | 77.31 | 76.07 | 76.95 | +0.88 |
| jar_kettle | 88.32 | 87.53 | 84.57 | −2.96 |
| door | 83.98 | 82.58 | 84.41 | +1.83 |
| cup | 96.55 | 95.39 | **96.62** | +1.23 |
| wall | 91.99 | 91.41 | 93.38 | +1.97 |
| bowl | 85.51 | 86.98 | **87.75** | +0.77 |
| shelf | 78.56 | 80.90 | 82.43 | +1.53 |
| **mAcc** | 88.14 | 87.45 | **88.18** | **+0.73** |
...
|--------|------|-----------|------|
| mIoU ≥ 80.71% | ☑ | **81.58%** | ☑ |
| bowl ≥ 78.91% | ☑ | **80.32%** | ☑（**高于 fix5k +0.25**） |
| window ≥ 80% | 建议 | **82.76%** | ☑（接近 balanced10k 82.91） |

**iter_2000 小结**：已达标；**已被 iter_4000 在 mIoU/bowl/window 上超过**（见 §12.5）。

#### 12.4.5 checkpoint 进度表

| checkpoint | mIoU | bowl | window | 备注 |
|------------|------|------|--------|------|
| iter_2000 | 81.58 | 80.32 | 82.76 | ☑ 已测 |
| **iter_4000** | **81.81** | **80.82** | **83.33** | ☑ 已测；**mIoU/bowl/window 最佳** |
| iter_6000 | 81.80 | 80.70 | 77.16 | ☑ 已测；**shelf 回升**，window 回落 |
| iter_8000 | 81.18 | 79.10 | 80.65 | ☑ 已测；**长训退化**，不推荐 |

### 12.5 实测：iter_4000（val 1000，2026-05-23）

**权重**：`outputs/trans10k_lass_mmscope_balanced_v2/iter_4000.pth`

#### 12.5.1 总体指标

| 指标 | 基线 | fix5k | balanced10k | v2@2k | **v2 @ 4k** | 4k−fix5k | 4k−2k |
|------|------|-------|-------------|-------|-------------|----------|-------|
| **mIoU** | 80.71 | 80.84 | 81.76 | 81.58 | **81.81** | **+0.97** | **+0.23** |
| aAcc | 96.07 | 95.92 | 96.15 | 96.05 | 96.03 | +0.11 | −0.02 |
| mAcc | 88.14 | 87.45 | 87.71 | 88.18 | **88.87** | **+1.42** | **+0.69** |

#### 12.5.2 各类 IoU（%）— 与 fix5k 全量对比

| 类别 | 基线 | fix5k | balanced10k | **v2 @ 4k** | 4k−fix5k | 4k−2k |
|------|------|-------|-------------|-------------|----------|-------|
| background | 96.71 | 96.37 | 96.45 | 96.37 | 0.00 | −0.07 |
| box | 71.47 | 71.44 | 71.86 | 72.47 | +1.03 | −0.24 |
| bottle | 87.77 | 86.51 | 88.20 | **88.37** | **+1.86** | **+1.91** |
| **window** | 66.62 | 76.27 | 82.91 | **83.33** | **+7.06** | **+0.57** |
| eyeglass | 92.85 | 90.81 | 92.01 | **91.77** | **+0.96** | **+2.19** |
| freezer | 73.90 | 73.46 | 73.48 | 73.30 | −0.16 | −0.67 |
| jar_kettle | 84.04 | 82.06 | 83.89 | 83.55 | +1.49 | +0.93 |
| door | 75.04 | 72.73 | 74.40 | 72.97 | +0.24 | 0.00 |
| cup | 90.91 | 90.19 | 90.96 | **91.51** | **+1.32** | **+0.54** |
| wall | 82.72 | 82.77 | 83.95 | **83.66** | +0.89 | +0.09 |
| **bowl** | 78.91 | 80.07 | 74.31 | **80.82** | **+0.75** | **+0.50** |
| shelf | 67.61 | 67.44 | 68.73 | 63.61 | −3.83 | −2.98 |
| **mIoU** | 80.71 | 80.84 | 81.76 | 81.58 | **81.81** | **+0.97** | **+0.23** |

**相对 fix5k（\|Δ\|>0.2%）**：↑ **8**（window、bowl、bottle、eyeglass、jar_kettle、cup、wall、box）；↓ **2**（freezer、shelf）。

**相对 balanced10k**：mIoU **+0.05**；bowl **+6.51**；window **+0.42** — **首次四指标均不低于 balanced10k（mIoU 略高）**。

#### 12.5.3 各类 Acc（%）— 与 fix5k 对比

| 类别 | fix5k | **v2 @ 4k** | 4k−fix5k |
|------|-------|-------------|----------|
| background | 98.33 | 97.78 | −0.55 |
| box | 79.44 | 81.91 | +2.47 |
| bottle | 87.97 | 89.49 | +1.52 |
| window | 86.71 | 88.65 | +1.94 |
| eyeglass | 96.04 | 96.87 | +0.83 |
| freezer | 76.07 | 77.11 | +1.04 |
| jar_kettle | 87.53 | 88.23 | +0.70 |
| door | 82.58 | 87.74 | +5.16 |
| cup | 95.39 | 95.28 | −0.11 |
| wall | 91.41 | 93.59 | +2.18 |
| bowl | 86.98 | 87.51 | +0.53 |
| shelf | 80.90 | 82.22 | +1.32 |
| **mAcc** | 87.45 | **88.87** | **+1.42** |

#### 12.5.4 小结（iter_4000）

| 验收项 | iter_2000 | **iter_4000** | 判定 |
|--------|-----------|---------------|------|
| mIoU ≥ 80.71% | 81.58 | **81.81** | ☑（**目前 v2 最高**） |
| bowl ≥ 78.91% | 80.32 | **80.82** | ☑（**高于 fix5k +0.75**） |
| window ≥ 80% | 82.76 | **83.33** | ☑ |

**注意**：**shelf IoU 63.61** 明显低于 fix5k（67.44）与 2k（66.59），选权重时需与 shelf 需求权衡。

**暂定（偏 window）**：`balanced_v2/iter_4000.pth`；**偏 shelf** 见 §12.6 iter_6000。全表见对比分析 §5.6～5.9。

### 12.6 实测：iter_6000（val 1000，2026-05-23）

**权重**：`outputs/trans10k_lass_mmscope_balanced_v2/iter_6000.pth`

#### 12.6.1 总体指标

| 指标 | 基线 | fix5k | balanced10k | v2@4k | **v2 @ 6k** | 6k−fix5k | 6k−4k |
|------|------|-------|-------------|-------|-------------|----------|-------|
| **mIoU** | 80.71 | 80.84 | 81.76 | 81.81 | **81.80** | **+0.96** | −0.01 |
| aAcc | 96.07 | 95.92 | 96.15 | 96.03 | **96.12** | **+0.20** | +0.09 |
| mAcc | 88.14 | 87.45 | 87.71 | 88.87 | **88.93** | **+1.48** | +0.06 |

#### 12.6.2 各类 IoU（%）— 与 fix5k 全量对比

| 类别 | 基线 | fix5k | balanced10k | v2@4k | **v2 @ 6k** | 6k−fix5k | 6k−4k |
|------|------|-------|-------------|-------|-------------|----------|-------|
| background | 96.71 | 96.37 | 96.45 | 96.37 | 96.51 | +0.14 | +0.14 |
| box | 71.47 | 71.44 | 71.86 | 72.47 | **72.97** | **+1.53** | +0.50 |
| bottle | 87.77 | 86.51 | 88.20 | 88.37 | 88.04 | **+1.53** | −0.33 |
| **window** | 66.62 | 76.27 | 82.91 | **83.33** | 77.16 | +0.89 | **−6.17** |
| eyeglass | 92.85 | 90.81 | 92.01 | 91.77 | **92.08** | **+1.27** | +0.31 |
| freezer | 73.90 | 73.46 | 73.48 | 73.30 | 73.39 | −0.07 | +0.09 |
| jar_kettle | 84.04 | 82.06 | 83.89 | 83.55 | **84.14** | **+2.08** | +0.59 |
| door | 75.04 | 72.73 | 74.40 | 72.97 | **73.34** | +0.61 | +0.37 |
| cup | 90.91 | 90.19 | 90.96 | 91.51 | **91.98** | **+1.79** | +0.47 |
| wall | 82.72 | 82.77 | 83.95 | 83.66 | 83.54 | +0.77 | −0.12 |
| **bowl** | 78.91 | 80.07 | 74.31 | **80.82** | **80.70** | **+0.63** | −0.12 |
| **shelf** | 67.61 | 67.44 | 68.73 | 63.61 | **67.73** | **+0.29** | **+4.12** |
| **mIoU** | 80.71 | 80.84 | 81.76 | 81.81 | **81.80** | **+0.96** | −0.01 |

**相对 fix5k（\|Δ\|>0.2%）**：↑ **9**（box、bottle、eyeglass、jar_kettle、door、cup、wall、bowl、shelf）；↓ **1**（freezer 略）；**window** 仍高于 fix5k（+0.89），但远低于 4k。

**相对 balanced10k**：mIoU **+0.04**；bowl **+6.39**；**window −5.75**（6k 在 window 上不如 balanced10k）。

#### 12.6.3 各类 Acc（%）— 与 fix5k 对比

| 类别 | fix5k | **v2 @ 6k** | 6k−fix5k |
|------|-------|-------------|----------|
| background | 98.33 | 97.98 | −0.35 |
| box | 79.44 | 77.99 | −1.45 |
| bottle | 87.97 | 89.44 | +1.47 |
| window | 86.71 | **90.42** | **+3.71** |
| eyeglass | 96.04 | 96.23 | +0.19 |
| freezer | 76.07 | 77.44 | +1.37 |
| jar_kettle | 87.53 | 88.32 | +0.79 |
| door | 82.58 | 86.85 | +4.27 |
| cup | 95.39 | **96.54** | +1.15 |
| wall | 91.41 | 92.83 | +1.42 |
| bowl | 86.98 | **89.91** | **+2.93** |
| shelf | 80.90 | **83.16** | **+2.26** |
| **mAcc** | 87.45 | **88.93** | **+1.48** |

#### 12.6.4 小结（iter_6000）

| 验收项 | iter_4000 | **iter_6000** | 判定 |
|--------|-----------|---------------|------|
| mIoU ≥ 80.71% | 81.81 | **81.80** | ☑（与 4k 基本持平） |
| bowl ≥ 78.91% | 80.82 | **80.70** | ☑（略低于 4k **−0.12**） |
| window ≥ 80% | 83.33 | 77.16 | ☑ 阈值内，但 **较 4k −6.17** |
| shelf ≥ 基线 67.61% | 63.61 | **67.73** | ☑（**v2 已测最高**，略高于基线 **+0.12**） |

**权衡**：6k 在 **mIoU / bowl** 与 4k 几乎持平，**shelf 回到基线/fix5k 水平**（67.73），但 **window IoU 从 83.33 跌至 77.16**。若路线 C 强依赖 **window 掩码**，仍优先 **4k**；若 **shelf** 与 **bowl** 并重、可接受 window 接近 fix5k，可考虑 **6k**。

### 12.7 实测：iter_8000（val 1000，2026-05-23）

**权重**：`outputs/trans10k_lass_mmscope_balanced_v2/iter_8000.pth`

#### 12.7.1 总体指标

| 指标 | 基线 | fix5k | balanced10k | v2@4k | v2@6k | **v2 @ 8k** | 8k−fix5k | 8k−4k |
|------|------|-------|-------------|-------|-------|-------------|----------|-------|
| **mIoU** | 80.71 | 80.84 | 81.76 | 81.81 | 81.80 | **81.18** | +0.34 | **−0.63** |
| aAcc | 96.07 | 95.92 | 96.15 | 96.03 | 96.12 | 95.99 | +0.07 | −0.04 |
| mAcc | 88.14 | 87.45 | 87.71 | 88.87 | 88.93 | **88.04** | +0.59 | −0.83 |

#### 12.7.2 各类 IoU（%）— 与 fix5k 全量对比

| 类别 | 基线 | fix5k | balanced10k | v2@4k | **v2 @ 8k** | 8k−fix5k | 8k−4k |
|------|------|-------|-------------|-------|-------------|----------|-------|
| background | 96.71 | 96.37 | 96.45 | 96.37 | 96.37 | 0.00 | 0.00 |
| box | 71.47 | 71.44 | 71.86 | 72.47 | 72.94 | **+1.50** | +0.47 |
| bottle | 87.77 | 86.51 | 88.20 | 88.37 | 88.00 | **+1.49** | −0.37 |
| **window** | 66.62 | 76.27 | 82.91 | **83.33** | 80.65 | +4.38 | **−2.68** |
| eyeglass | 92.85 | 90.81 | 92.01 | 91.77 | 90.59 | −0.22 | −1.18 |
| freezer | 73.90 | 73.46 | 73.48 | 73.30 | 73.40 | −0.06 | +0.10 |
| jar_kettle | 84.04 | 82.06 | 83.89 | 83.55 | 84.07 | **+2.01** | +0.52 |
| door | 75.04 | 72.73 | 74.40 | 72.97 | 71.35 | −1.38 | −1.62 |
| cup | 90.91 | 90.19 | 90.96 | 91.51 | 91.46 | **+1.27** | −0.05 |
| wall | 82.72 | 82.77 | 83.95 | 83.66 | 83.33 | +0.56 | −0.33 |
| **bowl** | 78.91 | **80.07** | 74.31 | **80.82** | 79.10 | **−0.97** | **−1.72** |
| **shelf** | **67.61** | 67.44 | 68.73 | 63.61 | 62.86 | −4.58 | −0.75 |
| **mIoU** | 80.71 | 80.84 | 81.76 | 81.81 | — | **81.18** | +0.34 | **−0.63** |

**相对 fix5k（\|Δ\|>0.2%）**：↑ **5**（box、bottle、window、jar_kettle、cup）；↓ **3**（bowl、door、shelf）。

**相对 4k/6k**：8k **未**实现「window + shelf 兼得」——mIoU/bowl 低于 4k/6k，shelf 仅略好于 4k、仍远低于基线/fix5k/6k。

#### 12.7.3 各类 Acc（%）— 与 fix5k 对比

| 类别 | fix5k | **v2 @ 8k** | 8k−fix5k |
|------|-------|-------------|----------|
| background | 98.33 | 98.04 | −0.29 |
| box | 79.44 | 79.15 | −0.29 |
| bottle | 87.97 | 89.52 | +1.55 |
| window | 86.71 | 89.15 | +2.44 |
| eyeglass | 96.04 | 96.56 | +0.52 |
| freezer | 76.07 | 77.12 | +1.05 |
| jar_kettle | 87.53 | 88.07 | +0.54 |
| door | 82.58 | 79.89 | −2.69 |
| cup | 95.39 | 95.89 | +0.50 |
| wall | 91.41 | 93.63 | +2.22 |
| bowl | 86.98 | 88.16 | +1.18 |
| shelf | 80.90 | 81.33 | +0.43 |
| **mAcc** | 87.45 | 88.04 | +0.59 |

#### 12.7.4 小结（iter_8000）

| 验收项 | iter_4000 | iter_6000 | **iter_8000** |
|--------|-----------|-----------|---------------|
| mIoU ≥ 80.71% | 81.81 | 81.80 | 81.18（**v2 已测最低**） |
| bowl ≥ fix5k 80.07% | 80.82 | 80.70 | **79.10（低于 fix5k）** |
| window ≥ 80% | 83.33 | 77.16 | 80.65 |
| shelf ≥ 基线 67.61% | 63.61 | 67.73 | **62.86（v2 已测最低）** |

**结论**：训满 8k 后 **整体回落**，**不推荐** 作为 v2 交付权重；**早停宜在 4k（偏 window）或 6k（偏 shelf）**。

### 12.8 balanced-v2 终选（2k / 4k / 6k / 8k 已齐）
...
| 2026-05-23 | §12：balanced-v2 配置、BowlAntiCupLoss、训练脚本；fix5k recipes 备份 |
| 2026-05-23 | §12.4：balanced-v2 iter_2000 test，与基线/fix5k/balanced10k 对比 |
